...
// ── Welcome Popup ─────────────────────────────────────────────────────────────
// ── Jai Shree Ram Greeting ────────────────────────────────────────────────────
function useJaiShreeRamGreeting() {
  useEffect(() => {
    if (sessionStorage.getItem("jsr_played")) return;
    sessionStorage.setItem("jsr_played", "1");
    const timer = setTimeout(() => {
      try {
        if (!window.speechSynthesis) return;
        const utterance = new SpeechSynthesisUtterance("Jai Shree Ram");
        utterance.rate = 0.8;
        utterance.pitch = 1.0;
        utterance.volume = 0.9;
        window.speechSynthesis.speak(utterance);
      } catch {
        // Silently ignore if Speech API is unavailable
      }
    }, 1000);
